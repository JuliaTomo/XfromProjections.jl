...
frames = images[:,:,collect(1:10:300)]
@info size(frames)
...
plot!(tracks[301][:,1], tracks[301][:,2], aspect_ratio=:equal, linewidth=5)
l = @layout [a b c d]
plot(p1, p2, p3, p4, layout = l, size=(2000,600), linewidth=5)
savefig(@sprintf "result_all_tv_20_rand%d" length(angles))
...
l = @layout [a b c d]
plot(p1, p2, p3, p4, layout = l, size=(2000,600), linewidth=5)
savefig(@sprintf "result_all_flow_20_rand%d" length(angles))
...
!isdir(path) && mkdir(path)
cd(path)
gif(anim, "reconstruction_flow_tv_20_rand.gif", fps = 1)
cd(cwd)
